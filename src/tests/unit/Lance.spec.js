...
describe('um lance sem valor minimo', () => {
  ...
  test('emitir lance quando o valor é maior que zero', () => {
    const wrapper = mount(Lance)
  ...
  test('emite o valor esperando de um lance valido', () => {
    const wrapper = mount(Lance)
    const input = wrapper.find('input')
    input.setValue(100)
    wrapper.trigger('submit')
...
describe('um lance com valor minimo', () => {
  ...
  test('não são aceitos lances com valor menor do que o valor informado', async () => {
    const wrapper = mount(Lance, {
      propsData: {
        lanceMinimo: 300
      }
